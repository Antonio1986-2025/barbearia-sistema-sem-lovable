...
const comissoes = {
  async gerar(barbeiroId, agendamentoId, comandaId, valorServico) {
    const valorComissao = valorServico * 0.5;
    const hoje = new Date();
    ...
    const result = await db.query(
      `INSERT INTO comissoes (barbeiro_id, agendamento_id, comanda_id, valor_servico, valor_comissao, semana_inicio, semana_fim)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [barbeiroId, agendamentoId, comandaId, valorServico, valorComissao, segunda.toISOString().split('T')[0], domingo.toISOString().split('T')[0]]
    );
  ...
  async resumoSemana(barbeiroId, dataInicio, dataFim) {
    ...
    return { ...result.rows[0], total_vales: vales.rows[0].total_vales };
  },
  async registrarVale(barbeiroId, valor, descricao = '') {
    const result = await db.query(
      `INSERT INTO vales (barbeiro_id, valor, descricao) VALUES ($1, $2, $3) RETURNING *`,
      [barbeiroId, valor, descricao]
    );
    return result.rows[0];
  },
